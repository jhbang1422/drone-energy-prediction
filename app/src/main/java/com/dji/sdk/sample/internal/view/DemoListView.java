        ...
        public void run(){
            try{
                ...
                while (true){
                    ...
                    buf = sdf.format(new Date(curr));

                    Aircraft air = (Aircraft) DJISampleApplication.getProductInstance();
                    BaseProduct bp = DJISampleApplication.getProductInstance();

                    air.getFlightController().setStateCallback(new FlightControllerState.Callback(){
                        @Override
                        public void onUpdate(FlightControllerState djiFlightState){
                            buf = addString(buf,"Xvelocity");
                            buf = addString(buf, String.valueOf(djiFlightState.getVelocityX()));
                            buf = addString(buf, "Yvelocity");
                            buf = addString(buf, String.valueOf(djiFlightState.getVelocityY()));
                            buf = addString(buf, "Zvelocity:");
                            buf = addString(buf, String.valueOf(djiFlightState.getVelocityZ()));
                        }
                    });

                    bp.getBattery().setStateCallback(new BatteryState.Callback(){
                        @Override
                        public void onUpdate(BatteryState djiBatteryState){
                            buf = addString(buf, "Remaining Percent:");
                            buf = addString(buf, String.valueOf(djiBatteryState.getChargeRemainingInPercent()));
                            buf = addString(buf, "Voltage:");
                            buf = addString(buf, String.valueOf(djiBatteryState.getVoltage()));
                            buf = addString(buf, "Current:");
                            buf = addString(buf, String.valueOf(djiBatteryState.getCurrent()));
                            buf = addString(buf, "Full Capacity:");
                            buf = addString(buf, String.valueOf(djiBatteryState.getFullChargeCapacity()));
                            buf = addString(buf, "Remaining Capacity:");
                            buf = addString(buf, String.valueOf(djiBatteryState.getChargeRemaining()));
